...
| 디렉토리 | 역할 |
|----------|------|
| `services/` | 비즈니스 로직 (sync, storage, ai-runner, terminal, scheduler, slack, email, 팀별 daily-report-scheduler/dm-reminder-scheduler) |
...
| `slack` | slack.handlers | 슬랙 웹훅 테스트, Bot Token 연결 테스트, 스레드 메시지 검색, DM 테스트 |
| `email` | email.handlers | Gmail OAuth 인증 시작/상태확인/연결해제, 리포트 이메일 전송 (Gmail API) |
