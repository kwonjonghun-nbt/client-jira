import { useAITaskStore } from '../../store/aiTaskStore';
import { countRunningTasks } from '../../utils/ai-tasks';

export default function FloatingAIButton() {
  const tasks = useAITaskStore((s) => s.tasks);
  const togglePanel = useAITaskStore((s) => s.togglePanel);
  const runningCount = countRunningTasks(tasks);

  // Only show when there are tasks
  if (tasks.length === 0) return null;

  return (
    <button
      type="button"
      onClick={togglePanel}
      className={`fixed top-4 right-4 z-50 w-10 h-10 rounded-full bg-purple-500 hover:bg-purple-600 text-white shadow-lg flex items-center justify-center cursor-pointer transition-colors ${
        runningCount > 0 ? 'animate-pulse' : ''
      }`}
      title="AI 작업 관리"
    >
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" />
      </svg>

      {/* Badge: running task count */}
      {runningCount > 0 && (
        <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
          {runningCount}
        </span>
      )}
    </button>
  );
}
