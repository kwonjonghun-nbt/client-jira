...
아래 "상태 전환 데이터 (JSON)"를 분석하여 팀의 프로세스 개선점을 도출해주세요.
상태 전환 데이터가 없는 경우 이 섹션은 생략해주세요.

### 병목 구간 분석
- 각 이슈의 bottleneck 필드를 확인하여, 가장 오래 걸리는 상태 전환 구간을 식별
- 팀 전체에서 공통적으로 병목이 되는 구간이 있는지 패턴 분석
- 병목 구간별 평균 소요시간과 최대 소요시간을 표로 정리

| 구간 | 이슈 수 | 평균 소요시간 | 최대 소요시간 | 해당 이슈 |
|------|---------|---------------|---------------|-----------|

### 비정상 전환 패턴
- flags에 'single_jump'가 있는 이슈: 중간 단계(테스트, 리뷰 등) 없이 바로 완료 → 프로세스 준수 여부 점검
- flags에 'reverse_transition'이 있는 이슈: 역방향 전환(완료→진행중 등) → 반려/재작업 사유 분석
- flags에 'rapid_transition'이 있는 이슈: 작업 단계에서 5분 이내 전환 → 이미 작업을 끝내놓고 Jira 상태만 일괄 변경한 의심 사례. 실제 해당 단계에서 작업이 이루어졌는지 확인 필요. 어떤 전환이 5분 이내였는지 구체적으로 명시
- flags에 'no_transitions'가 있는 이슈: 상태 변경 이력 없음 → 티켓 관리 미흡

| 이슈 키 | 제목 | 패턴 | 상세 내용 | 개선 제안 |
|----------|------|------|-----------|-----------|

### 프로세스 개선 제안
- 병목 해소를 위한 구체적 제안 (리뷰어 부족, 테스트 환경 문제 등)
- 프로세스 미준수 이슈에 대한 개선 방안
- 형식적 상태 변경이 발견된 경우, Jira 상태를 실시간으로 업데이트하는 습관에 대한 제안
...
8. 상태 전환 데이터의 durationMs는 밀리초 단위이므로, 표시할 때는 시간/일 단위로 변환`;
}
