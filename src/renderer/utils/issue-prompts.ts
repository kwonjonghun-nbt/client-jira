...
export function buildPrompt(issue: NormalizedIssue): string {
  ...
  lines.push(
    ...
    '',
    '### 1. 티켓 설명 보완',
    '현재 설명을 분석하여 위 템플릿에 맞게 **완성된 설명**을 Markdown 문법으로 작성해주세요.',
    ...
    '아래 순서로 결과를 정리해주세요:',
    '',
    '1. **보완된 티켓 설명** — 위 템플릿에 맞춰 Markdown 문법으로 바로 붙여넣기 할 수 있는 완성본',
    '2. **설정 변경 제안** — 표 형태 (항목 | 현재값 | 권장값 | 사유)',
    '3. **Subtask 목록** — 위 형식에 맞춰 각 Subtask 상세',
    '4. **요약** — 주의사항',
    '',
    '**중요: 결과를 코드블록(```markdown ... ```)으로 감싸지 마세요. Markdown 문법을 그대로 출력해주세요.**',
  );
